...
public interface PostVoteRepository extends JpaRepository<PostVote, Long> {
    ...
    @Query(value = "update table post_vote set vote_type = 'UP_VOTE' where user_id = ?1 and post_id = ?2", nativeQuery = true)
    void updateVoteTypeToUpVote(long id, long postId);

    @Transactional
    @Modifying(clearAutomatically = true)
    @Query(value = "update table post_vote set vote_type = 'DOWN_VOTE' where user_id = ?1 and post_id = ?2", nativeQuery = true)
